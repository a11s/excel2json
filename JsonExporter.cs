...
namespace excel2json
{
    ...
    {
        string mContext = "";
        Newtonsoft.Json.JsonSerializerSettings s = new JsonSerializerSettings();
        ...
        public JsonExporter(DataTable sheet, int headerRows, bool lowcase, bool exportArray, bool nonAscii)
        {
            ...
            if (exportArray)
            {
                convertArray(sheet, headerRows, lowcase, nonAscii);
            }
            else
            {
                convertDict(sheet, headerRows, lowcase, nonAscii);
            }
        }

        private void convertArray(DataTable sheet, int headerRows, bool lowcase, bool nonAscii)
        {
            ...
            if (nonAscii)
            {
                s.StringEscapeHandling = Newtonsoft.Json.StringEscapeHandling.EscapeNonAscii;
            }
            else
            {
                s.StringEscapeHandling = Newtonsoft.Json.StringEscapeHandling.Default;
            }
            //-- convert to json string
            mContext = JsonConvert.SerializeObject(values, Formatting.Indented, s);
        }

        /// <summary>
        /// 以第一列为ID，转换成ID->Object的字典对象
        /// </summary>
        private void convertDict(DataTable sheet, int headerRows, bool lowcase, bool nonAscii)
        {
            ...
            //-- convert to json string
            mContext = JsonConvert.SerializeObject(importData, Formatting.Indented, s);
        }
